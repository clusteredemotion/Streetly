...
export default function SetupPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    if (!token) {
      setError("This setup link is missing its token. Please use the link from your welcome email.");
      return;
    }
    ...
    try {
      const result = await setupPasswordMutation.mutateAsync({ data: { token, newPassword } });
      if (result.token) {
      ...
      setError(err?.data?.error ?? "Could not set your password. The link may be invalid or expired.");
    }
  };
        ...
        <div className="w-full max-w-sm">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Set a password for your Streetly account to get started.
            </p>
          </div>
          ...
          <div className="bg-card border rounded-2xl p-6 shadow-sm">
            {success ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Button type="submit" className="w-full" disabled={setupPasswordMutation.isPending}>
                  {setupPasswordMutation.isPending ? "Setting password..." : "Set Password"}
                </Button>
              </form>
            )}
          </div>
